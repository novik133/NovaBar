...
namespace Indicators {
    ...
    public class Network : Gtk.Button {
        ...
        public Network() {
            ...
            Debug.log("Network", "Creating Network indicator...");
            icon = new Gtk.Image.from_icon_name("network-wireless-symbolic", Gtk.IconSize.MENU);
            add(icon);
            Debug.log("Network", "Network indicator icon created");
            
            // Initialize NetworkManager asynchronously to prevent blocking
            Idle.add(() => {
                setup_nm();
                return false;
            });
            ...
            Debug.log("Network", "Network indicator constructor complete");
        }
        ...
        private void setup_nm() {
            Debug.log("Network", "Initializing NetworkManager client...");
            try {
                nm_client = new NM.Client(null);
                Debug.log("Network", "NetworkManager client created successfully");
                find_wifi_device();
                update_icon();
                nm_client.notify["connectivity"].connect(update_icon);
                Debug.log("Network", "NetworkManager setup complete");
            } catch (Error e) {
                Debug.log("Network", "NetworkManager not available: %s".printf(e.message));
